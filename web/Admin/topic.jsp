...
<html>
    ...
    <body>
        ...
        <center><h1 style="color: green;">${message}</h1></center>
        </c:if>
    <h1>Hello,   ${sessionScope.userName}
        <a href="<%=request.getContextPath()%>/Admin/adminModule.jsp">Home</a>,  <a href="<%=request.getContextPath()%>/Logout">Logout</a>,  <a href="<%=request.getContextPath()%>/Admin/topic.jsp">Refresh</a>
    </h1>
    <c:catch var="ex">
        ...
        <c:if test="${pageNo ne null and not empty pageNo}">
            <c:set var="pageNumber" value="${pageNo}" />
        </c:if>
        ...
        <table> 
            ...
            <c:forEach items="${topic.rows}" var="t" begin="${start}" end="${stop}">
                <tr>
                    ...
                    <td>${t.image_url}</td>
                    <td><a href="<%=request.getContextPath()%>/Admin/editTopic.jsp?id=${t.unique_id}&p=${pageNumber}">Edit</a></td>
                </tr>
            </c:forEach>
        ...
        <c:if test="${pageNumber gt 1}">
            <a href="<%=request.getContextPath()%>/Admin/topic.jsp?p=${pageNumber - 1}">Previous</a>
        </c:if>
        <c:forEach begin="1" end="${numberOfPages}" var="i">
            <c:choose>
                <c:when test="${i!=pageNumber}">
                    <a href="<%=request.getContextPath()%>/Admin/topic.jsp?p=<c:out value="${i}"/>"><c:out value="${i}"/></a>
                </c:when>
                <c:otherwise>
        ...
        <c:if test="${pageNumber lt numberOfPages}">
            <a href="<%=request.getContextPath()%>/Admin/topic.jsp?p=${pageNumber + 1}">Next</a>
        </c:if>
    </c:catch>
